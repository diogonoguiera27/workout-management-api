...
describe("CreateWorkoutPlan load", () => {
  ...
  const users = Array.from({ length: totalUsers }, (_, index) =>
    createBenchmarkUserContext(`load-user-${index}`),
  );
  ...
  it("should handle multiple workout plan creations concurrently", async () => {
    ...
    const results = await Promise.all(
      users.map((user, index) =>
        sut.execute(
          buildWorkoutPlanInput(user.userId, `Load plan ${index}-${Date.now()}`),
        ),
      ),
    );
    ...
    console.info(
      `[load] ${totalUsers} concurrent CreateWorkoutPlan operations completed in ${elapsedInMs.toFixed(2)}ms`,
    );
  }, 30000);
});

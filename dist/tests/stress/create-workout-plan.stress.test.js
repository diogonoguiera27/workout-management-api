...
describe("CreateWorkoutPlan stress", () => {
    ...
    const totalUsers = 18;
    const users = Array.from({ length: totalUsers }, (_, index) => createBenchmarkUserContext(`stress-user-${index}`));
    beforeAll(async () => {
    ...
    it("should sustain repeated workout plan creation bursts without failures", async () => {
        ...
        for (const batchSize of batchSizes) {
            ...
            cursor += batchSize;
            const batchResults = await Promise.allSettled(batchUsers.map((user, index) => sut.execute(buildWorkoutPlanInput(user.userId, `Stress plan ${cursor}-${index}-${Date.now()}`))));
        ...
        console.info(`[stress] ${totalUsers} CreateWorkoutPlan operations across bursts completed in ${elapsedInMs.toFixed(2)}ms`);
    }, 45000);
});

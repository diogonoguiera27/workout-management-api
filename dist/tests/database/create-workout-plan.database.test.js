...
describe("CreateWorkoutPlan database", () => {
    ...
    it("should persist the full workout graph and query it efficiently", async () => {
        ...
        const createdPlan = await sut.execute(buildWorkoutPlanInput(user.userId, `Database plan ${Date.now()}`));
        ...
        console.info(`[database] create=${createElapsedInMs.toFixed(2)}ms query=${queryElapsedInMs.toFixed(2)}ms`);
    }, 20000);
});
